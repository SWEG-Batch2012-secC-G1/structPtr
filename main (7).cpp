...
void create_product()
	{
	  ofstream f;
	   /* cout<<"how many product do you want to create: "<<endl;
	    cin>>quant;
	    
	    for(i=0;i<quant;i++)
	    produc.push_back(i);*/
	   	f.open("database.dat",ios::out|ios::app);
		...
		cin>>produc.discount;
		f<<produc.product_num<<produc.product_name<<produc.price<<produc.quantity<<produc.discount;

		f.write((char*)&produc,sizeof(produc));
		f.close();
...
void show_all_product()
{
	...
	cout<<endl<<"\t\tRECORDS.";
	cout<<endl<<"\t\t@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n";
	f.open("database.dat",ios::in);
	f.read((char*)&produc,sizeof(produc));
	while(!f.eof())
	{
	f>>produc.product_num>>produc.product_name>>produc.price>>produc.quantity>>produc.discount;
...
void display_record(int num)
{
	bool found=false;
	f.open("database.dat",ios::in);
	while(f.read((char*)&produc,sizeof(produc)))
	{
...
void edit_product()
{
	...
	f.open("database.dat",ios::in|ios::out);
	while(f.read((char*)&produc,sizeof(produc)) && found==false)
	{
		if(getProduct<int>()==num)
		{
			show_product();
...
while(f.read((char*)&produc,sizeof(product< t1,t2,t3>)))
	{
